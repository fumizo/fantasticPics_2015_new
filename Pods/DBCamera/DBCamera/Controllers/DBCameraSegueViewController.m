...
- (id) initWithImage:(UIImage *)image thumb:(UIImage *)thumb boolsecond:(BOOL)boolsecond
{
    ...
    if (self) {
        ...
        [[NSUserDefaults standardUserDefaults] setObject:imageData forKey:@"cache"];
...
- (void) saveImage
{
    ...
    if ( [_delegate respondsToSelector:@selector(camera:didFinishWithImage:withMetadata:)] ) {
        ...
        NSLog(@"not found");
        UIImage *transform = [_filterMapping[@(_selectedFilterIndex.row)] imageByFilteringImage:editpic];
        //[_delegate camera:self didFinishWithImage:transform withMetadata:self.capturedImageMetadata];
        // 画像をNSUserDefaultsへ保存する
        NSData *imageData = UIImagePNGRepresentation(transform);
